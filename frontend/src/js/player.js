...
export function initPlayer(socket) {
    ...
    const precacheNextTracks = () => {
        ...
        if (globalQueue.length > 0) {
            nextUrl = `/music/${encodePath(globalQueue[0].path)}`;
        } else if (isShuffle && shuffleQueue.length > 0) {
            const idx = shuffleQueue[shuffleQueue.length - 1];
            nextUrl = `/music/${encodePath(currentPlaylist[idx].path)}`;
        } else {
            ...
            if (currentIndex !== -1) {
                ...
                if (nextIndex < currentPlaylist.length) {
                    nextUrl = `/music/${encodePath(currentPlaylist[nextIndex].path)}`;
                } else if (isRepeat !== 0) { // wrap around if repeat is on
                    nextUrl = `/music/${encodePath(currentPlaylist[0].path)}`;
                }
            }
        }
        ...
        if (nextUrl) {
            ...
            if (nextAudioPreloader.src !== absoluteNextUrl) {
                ...
                // Force Android Chrome to actively buffer the upcoming file even if the tab is backgrounded.
                // Browsers often ignore <audio preload="auto"> for background tabs to save battery.
